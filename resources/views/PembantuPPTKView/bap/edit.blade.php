...
@section('content')
    <div class="container">
        ...
        <form action="{{ route('PembantuPPTKView.bap.update', ['id' => $baps->id]) }}" method="POST" enctype="multipart/form-data">
            @csrf
            @method('PUT')
    ...
    <select name="dpa_id" id="dpa_id" class="form-control" required disabled>
        ...
        @endforeach
            ...
            <div class="form-group">
                ...
                <input type="text" class="form-control @error('nomor') is-invalid @enderror" id="nomor" name="nomor" value="{{ old('nomor', $baps->nomor) }}" @if(Auth::user()->hasRole('PPTK')) disabled @endif>
                @error('nomor')
            ...
            <div class="form-group">
                ...
                <input type="date" class="form-control @error('tanggal') is-invalid @enderror" id="tanggal" name="tanggal" value="{{ old('tanggal', $baps->tanggal) }}" @if(Auth::user()->hasRole('PPTK')) disabled @endif>
                @error('tanggal')
                    <span class="invalid-feedback">{{ $message }}</span>
                @enderror
            </div>
            ...
            <div class="form-group">
                ...
                <textarea class="form-control @error('keterangan') is-invalid @enderror" id="keterangan" name="keterangan" @if(Auth::user()->hasRole('PPTK')) disabled @endif>{{ old('keterangan', $baps->keterangan) }}</textarea>
                @error('keterangan')
                    <span class="invalid-feedback">{{ $message }}</span>
                @enderror
            </div>
...
<div class="form-group">
    ...
    @if($baps->upload_dokumen)
        <p>Current file: {{ $baps->upload_dokumen }}</p>
    @endif
